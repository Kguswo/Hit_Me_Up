# Hit Me Up 

<img width="1029" height="395" alt="Hit Me Up 로고" src="https://github.com/user-attachments/assets/b443766b-bb22-4240-8325-29b6dcf34dc5" />

<br/><br/>

원하는 URL의 방문자 카운터를 쉽고 빠르게 만들어보세요!

뱃지를 생성하고 단 한 줄 코드를 추가하면 자동으로 방문자 수를 추적할 수 있어요.

![GitHub stars](https://img.shields.io/github/stars/Kguswo/Hit_Me_Up?style=social) ![GitHub watchers](https://img.shields.io/github/watchers/Kguswo/Hit_Me_Up?style=social) [![GitHub release (latest by date)](https://img.shields.io/github/v/release/Kguswo/Hit_Me_Up)](https://github.com/Kguswo/Hit_Me_Up/releases) [![Quality Gate Status](https://sonarcloud.io/api/project_badges/measure?project=Kguswo_Hit_Me_Up&metric=alert_status)](https://sonarcloud.io/summary/new_code?id=Kguswo_Hit_Me_Up) [![License](https://img.shields.io/github/license/Kguswo/Hit_Me_Up?color=yellow)](https://github.com/Kguswo/Hit_Me_Up/blob/develop/LICENSE)

[![Hit Me Up 방문자 수 뱃지 예시](https://hitmeup-backend-593087166771.asia-northeast1.run.app/api/count/increment?url=https%3A%2F%2Fgithub.com%2FKguswo%2FHit_Me_Up&title=Hit%20Me%20Up&title_bg=555555&count_bg=79c83d&edge_flat=false)](https://hitmeup-backend-593087166771.asia-northeast1.run.app)
<br/>

## 🌟 프로젝트 소개

**Hit Me Up**은 방문자 수를 특정 URL별로 실시간 집계하여 SVG 뱃지로 보여주는 간단하고 가벼운 방문자 카운터 서비스입니다.

한 줄 코드만 추가하면 GitHub README, Notion, 블로그 등 어디에나 쉽게 방문자 수를 표시할 수 있습니다.

<br/>

<p align="center">
    <a href="https://hit-me-up-336ec.web.app/">
        <img src="https://img.shields.io/badge/🎯_Hit_Me_Up_바로가기-72ba2a?style=for-the-badge" alt="Try Now"/>
</p>

<br/>

## 🚀 주요 기능

- 단 한 줄 코드 삽입으로 쉽고 빠르게 

- URL별 실시간 방문자 수 자동 집계  

- SVG 형식의 커스텀 가능한 방문자 카운터 뱃지 생성  

- 페이지 성능에 영향 없이 가볍게  

- 무료 & 오픈소스 프로젝트

<br/>

## 🛠️ 기술 스택

![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge&logo=kotlin&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black)
![Google Cloud](https://img.shields.io/badge/Google_Cloud-4285F4?style=for-the-badge&logo=google-cloud&logoColor=white)

<br/>

## ⚙️ 사용 방법

1. [Hit Me Up 서비스 페이지](https://hit-me-up-336ec.web.app/) 접속

2. 방문자 수를 추적려는 URL과 배지 옵션(텍스트, 색상 등)을 설정  

3. `GENERATE BADGE` 버튼 클릭

    <img width="1634" height="878" alt="Generating Badge" src="https://github.com/user-attachments/assets/9b7c2bc3-439c-4f77-b74c-d4a428297ade" />

4. 생성된 코드를 자신의 GitHub README, Notion, 웹 사이트 등 원하는 곳에 삽입  

    <img width="1646" height="1812" alt="Code View" src="https://github.com/user-attachments/assets/e389b3a0-40b8-45cd-8426-2785c80bc1f3" />

새로운 방문이 생길 때마다 방문자 수가 자동으로 증가합니다.

<br/>

## 🎯 주요 특징

- 방문자 수 카운터는 URL마다 분리하여 관리하며, Firestore 트랜잭션으로 데이터의 일관성을 보장  

- 미리보기 모드(preview)를 제공해 배지 생성 시 방문자 수 증가가 불가능하도록 설계되었습니다. 

- Firebase Hosting과 Cloud Run 연동을 통해 정적 파일과 백엔드 API를 분리 배포하였습니다. 

- Cloud Run을 사용한 적절한 리전 설정 및 도메인 매핑을 통한 안정적인 서비스 제공합니다.

<br/>

## 🤝 기여

모든 형태의 기여를 환영합니다! 이슈 등록 및 PR은 언제나 열려있습니다.  
함께 성장하는 오픈소스 커뮤니티에 참여해 주세요. 이메일도 환영합니다.

Email: nowalex322@gmail.com 

감사합니다!

<br/>

## 📜 라이선스

본 프로젝트는 [MIT 라이선스](https://github.com/Kguswo/Hit_Me_Up/blob/develop/LICENSE)를 따릅니다.

<br/>

## 🙋‍♂️ 소셜

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-Kguswo-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Kguswo)
&nbsp;&nbsp;&nbsp;
[![Article](https://img.shields.io/badge/✍️_Article-Hit_Me_Up-FF5722?style=for-the-badge&logoColor=white)](https://kguswo.github.io/posts/README.md-%EB%B0%A9%EB%AC%B8%EC%9E%90-%EC%88%98-%EC%B9%B4%EC%9A%B4%ED%8A%B8-%EC%84%9C%EB%B9%84%EC%8A%A4/)
&nbsp;&nbsp;&nbsp;
[![LinkedIn](https://img.shields.io/badge/LinkedIn-HyeonJae_Kim-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/hyeon-jae-kim-64b417299)

</div>

